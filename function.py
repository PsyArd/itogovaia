...
def add():
    ...
    print('Добавление заметки завершено...')
    ...
    for notes in array:
        if id == Note.Note.get_id(notes):
            ...
            if text == 'edit':
                ...
                print('Изменение заметки завершено...')
            if text == 'del':
                array.remove(notes)
                print('Удаление заметки завершено...')
            if text == 'show':
                print(Note.Note.map_note(notes))
    if logic == True:
        print('Такой заметки не существует, введен неверный id')
    file_operation.write_file(array, 'a')